...
export class ProductService {
    constructor(
        ...
        async getImage (): Promise<any>{
            try {
                const res = await axios.get('http://127.0.0.1:1337/api/product-cards?populate=*')
        ...
        async findAllProductWithImage (): Promise<ProductEntity[]> {
            ...
            const merge = findAllProducts.map((item) => {
                ...
                return{
                    ...item,
                    image_product: image ? `http:localhost:1337${ image.url }`: '' 
                }
            })
            return merge
        }
